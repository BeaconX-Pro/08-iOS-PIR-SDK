...
@implementation MKCQScanSearchButtonModel
@end

@interface MKCQScanSearchButton ()
...
@implementation MKCQScanSearchButton
...
- (void)clearButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(cq_scanSearchButtonClearMethod)]) {
        [self.delegate cq_scanSearchButtonClearMethod];
    }
}

- (void)searchButtonPressed{
    if ([self.delegate respondsToSelector:@selector(cq_scanSearchButtonMethod)]) {
        [self.delegate cq_scanSearchButtonMethod];
    }
}

#pragma mark - setting
- (void)setDataModel:(MKCQScanSearchButtonModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKCQScanSearchButtonModel.class]) {
        return;
    }
...
- (UIImageView *)searchIcon{
    if (!_searchIcon) {
        ...
        _searchIcon.image = LOADICON(@"MKBeaconPirSensor", @"MKCQScanSearchButton", @"cq_searchGrayIcon.png");
    }
    return _searchIcon;
}
...
- (UIButton *)clearButton{
    if (!_clearButton) {
        ...
        [_clearButton setImage:LOADICON(@"MKBeaconPirSensor", @"MKCQScanSearchButton", @"cq_clearButtonIcon.png") forState:UIControlStateNormal];
